TERMUX_PKG_HOMEPAGE=https://github.com/novaterm-org/nvterm-exec
TERMUX_PKG_DESCRIPTION="NovaTerm exec: LD_PRELOAD library for W^X bypass and proper execution environment"
TERMUX_PKG_LICENSE="Apache-2.0"
TERMUX_PKG_MAINTAINER="@novaterm"
TERMUX_PKG_VERSION=1:2.4.0
TERMUX_PKG_REVISION=1
# Builds from the same termux-exec source but with com.nvterm paths baked in.
# The source is Apache-2.0 licensed and fully open source.
...
TERMUX_PKG_BUILD_DEPENDS="nvterm-core-static"
...
termux_step_strip_elf_symbols() {
	...
	\( \
		...
		\( ! -path "./libexec/installed-tests/nvterm-exec/*" \) \
	\)
}

termux_step_post_massage() {
	# Compile runtime binary-tests for API level 28 if default is less than it.
	if [[ "$TERMUX_PKG_API_LEVEL" -lt 28 ]]; then
		...
		printf "\n%s\n" "Building libnvterm-exec_nos_c_tre_runtime-binary-tests for TERMUX_PKG_API_LEVEL '$TERMUX_PKG_API_LEVEL'"
		...
		local TERMUX_EXEC__TESTS__TESTS_PATH="$TERMUX_PKG_MASSAGEDIR/$TERMUX_PREFIX/libexec/installed-tests/nvterm-exec"
		local LIBTERMUX_EXEC__NOS__C__TESTS_PATH="$TERMUX_EXEC__TESTS__TESTS_PATH/lib/nvterm-exec_nos_c/tre"

		mkdir -p "$LIBTERMUX_EXEC__NOS__C__TESTS_PATH/bin"
		...
		install -m700 build/output/usr/libexec/installed-tests/termux-exec/lib/termux-exec_nos_c/tre/bin/libtermux-exec_nos_c_tre_runtime-binary-tests-fsanitize28 \
			"$LIBTERMUX_EXEC__NOS__C__TESTS_PATH/bin/libnvterm-exec_nos_c_tre_runtime-binary-tests-fsanitize28"
		$TERMUX_ELF_CLEANER --api-level 28 "$LIBTERMUX_EXEC__NOS__C__TESTS_PATH/bin/libnvterm-exec_nos_c_tre_runtime-binary-tests-fsanitize28"
		...
		install -m700 build/output/usr/libexec/installed-tests/termux-exec/lib/termux-exec_nos_c/tre/bin/libtermux-exec_nos_c_tre_runtime-binary-tests-nofsanitize28 \
			"$LIBTERMUX_EXEC__NOS__C__TESTS_PATH/bin/libnvterm-exec_nos_c_tre_runtime-binary-tests-nofsanitize28"
		$TERMUX_ELF_CLEANER --api-level 28 "$LIBTERMUX_EXEC__NOS__C__TESTS_PATH/bin/libnvterm-exec_nos_c_tre_runtime-binary-tests-nofsanitize28"
		printf "%s\n\n" "Install libnvterm-exec_nos_c_tre_runtime-binary-tests for TERMUX_PKG_API_LEVEL '$TERMUX_PKG_API_LEVEL' successful"
	fi
}
